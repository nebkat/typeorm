...
export class RdbmsSchemaBuilder implements SchemaBuilder {
            ...
            const dropQueries = table.indices
                .filter(tableIndex => {
                    ...
                    if (indexMetadata) {
                        ...
                        if (this.connection.driver.abilities.fullTextColumnType && indexMetadata.isFulltext !== tableIndex.isFulltext)
                            return true;
    ...
    protected async dropOldChecks(): Promise<void> {
        if (!this.connection.driver.abilities.checkConstraints) return;
    ...
    protected async dropOldExclusions(): Promise<void> {
        ...
        if (!this.connection.driver.abilities.exclusionConstraints) return;
    ...
    protected async updateExistColumns(): Promise<void> {
        for (const metadata of this.entityToSyncMetadatas) {
            ...
            if (this.connection.driver.abilities.uniqueConstraints) {
                for (const changedColumn of changedColumns) {
